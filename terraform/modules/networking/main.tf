...
# ALB SG
# HTTP traffic from ALB to nodes
resource "aws_security_group" "alb-sg" {
  name        = "note-app-ALB-sg"
  description = "Allows public HTTP traffic to the ALB and forwards it to the nodes."
  vpc_id      = aws_vpc.vpc.id
...
locals {
  ingress_rules = {
    k8s_nodes_from_alb = {
      ...
      to_port     = 32767
      protocol    = "tcp"
      source_sg   = aws_security_group.alb_sg.id
    },
    k8s_nodes_api = {
      from_port   = 6443
      to_port     = 6443
      protocol    = "tcp"
      source_sg   = aws_security_group.k8s_nodes_sg.id
    },
    k8s_nodes_etcd = {
      from_port   = 2379
      to_port     = 2380
...
# Uses for_each to create multiple security group rules dynamically.
